...
**Runnable.** Scenarios built from `run`, `assert`, `http`, `llm`, `foreach` and `notify` steps execute end to end: `baton run`, `resume`, `runs`, `validate`, `schema`. The [weekly report example](examples/weekly-report.yaml) is the current acceptance scenario — a foreach over projects through the GitLab pack, a model digest against a JSON schema, and a notification, cached so that a repeated run of the same week spends no tokens. `agent` steps run too, with the `fake` engine and the Claude Code adapter: the runner prepares the workspace, serves the step's `commands` as tools through the MCP gateway and takes the result from `submit_result`. The gateway also serves the api, git, fetch and state tools of the step's profile; the filesystem tools are still missing, so an agent cannot yet read a file it was not shown by a command.
