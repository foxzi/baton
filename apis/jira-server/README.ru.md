...
`comment` (`POST /rest/api/2/issue/{key}/comment`) выразим, и пак `jira`
его несёт, но personal access token, на котором проверялся этот пак,
предназначен только для чтения, поэтому ни одного запроса к нему на живом
сайте так и не было сделано. Он появится здесь, как только кто-то сможет
его проверить; до тех пор пак поставляет только проверенное. Сценарий,
которому нужно прокомментировать задачу в Server, отправляет комментарий
