...
`baton apis validate` воспроизводит пример так же, как это делает запуск:
для операции с пагинацией (`search_all.json`, `search_summary.json`,
`list_boards.json`, `list_board_issues.json`) файл — сырое тело одной
страницы, к нему применяется `pagination.items`, и трансформу подаются
элементы этой страницы. Поэтому записать новый пример — это обычный запрос
одной страницы к API, без ручной доработки.
