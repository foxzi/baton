...
- The gateway is an MCP server in a `config.toml` written into the run's own `CODEX_HOME`. The bearer token is passed by name (`bearer_token_env_var = "BATON_GATEWAY_TOKEN"`), never written into the file. `default_tools_approval_mode = "approve"` on that server auto-approves its tools: with approvals off, a call that asks for one fails instead of waiting
...
- Process environment: empty, plus `PATH`, `HOME` and `CODEX_HOME` (the same temporary directory, so no user configuration or cached credential reaches the run), and the declared `env`. The provider credential therefore has to be named in the step's `env`, unless the step sets `inherit_auth`
- `inherit_auth: true` copies the user's `auth.json` from their `CODEX_HOME` (`~/.codex` by default) into the run's own, which is what makes a subscription login work in place of an API key. It is a copy, so a run refreshing the access token cannot damage the login; a step that asks for credentials that are not there fails as a config error. Any engine other than `codex` refuses the field
