...
`baton validate scenario.yaml` и проверка перед `baton run` выводят сообщения
вида `error: scenario.yaml:8: budget.tokens: must not be negative`.
После файла и строки указаны путь поля и объяснение; для ошибок шагов
добавляется `(step build)`, если известен ID шага. Для неизвестного типа
входного параметра или источника секрета перечисляются допустимые значения.

Номер строки указывает на поле или содержащее его объявление, не обязательно
на отдельное значение. Для отсутствующего поля может использоваться строка
ближайшего известного родителя; если расположение неизвестно, номер опускается.
Ошибки разбора YAML сохраняют формат парсера. Исправьте указанные поля
и повторите `baton validate` перед запуском сценария.
