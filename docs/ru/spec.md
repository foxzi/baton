...
- Шлюз подключается как MCP-сервер в `config.toml`, который пишется в собственный `CODEX_HOME` прогона. Bearer-токен передаётся по имени переменной (`bearer_token_env_var = "BATON_GATEWAY_TOKEN"`), в файл он не попадает
...
- Окружение процесса: пустое, плюс `PATH`, `HOME` и `CODEX_HOME` (один и тот же временный каталог, чтобы в прогон не попали пользовательская конфигурация и сохранённые учётные данные), и объявленный `env`. Ключ провайдера, соответственно, должен быть назван в `env` шага
