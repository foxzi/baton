...
Ключ: `sha256(нормализованное определение шага без id и when + отрендеренные входы + хеши файлов промптов/схем/навыков + engine + model + версия baton)`. Кеш — каталог `cache/` рядом с `runs/`, содержимое — `output.json` и артефакты.
...
- [ ] Ни один секрет не появляется в `runs/`, логах, уведомлениях, аудит-логе (тест: секрет со случайным значением, grep по всему каталогу после прогона)
- [ ] Шаблон с `{{ .secrets.x }}` в промпте не проходит валидацию
- [ ] Процесс агента не имеет в окружении ничего, кроме аллоулиста (тест: fake-движок пишет `os.Environ()` в файл)
- [ ] Агент не может вызвать инструмент, не объявленный для шага (тест через fake: вызов чужого инструмента → `policy`)
- [ ] `commands` не интерпретируют метасимволы shell (тест: аргумент `; echo pwned` отклоняется `pattern`, аргумент `$(id)` при разрешающем pattern передаётся буквально)
- [ ] `fetch` на домен вне аллоулиста → `policy`; редирект на домен вне аллоулиста → `policy`
- [ ] Путь `../x` и `/etc/passwd` в аргументах команд и `fs` → `policy`
- [ ] `.git/config` после подготовки workspace не содержит токенов
- [ ] `http` с `POST` без `dedupe_key` не ретраится при `transient`
- [ ] `budget` не ретраится ни при каких настройках `retry`
- [ ] Пак с расхождением `sha256` или без пина версии не загружается
- [ ] Токен `path`-авторизации отсутствует в URL внутри логов, `events.jsonl`, `tool-calls.jsonl` и сообщениях об ошибках HTTP
- [ ] Токены `exchange` не попадают в кеш шагов и в `runs/`
- [ ] Агент не может вызвать операцию пака без `readonly: true`, даже если она перечислена в `tools.apis` (валидация) и даже при прямом обращении к шлюзу (рантайм → `policy`)
- [ ] jq-трансформ с `env`, `input`, `$__loc__` отклоняется при загрузке пака
...
Документация: README, справочник схемы (генерируется из JSON Schema), три примерных сценария (ревью, отчёт, триаж), `commands/go.yaml`, `commands/node.yaml`. Релизная сборка через goreleaser, бинарники для linux/amd64 и linux/arm64.

## 16. Открытые вопросы

1. **Имя.** `baton` занято в нише несколькими проектами; кандидат на замену — `segue`. Решить до M5, чтобы не переименовывать бинарник и конфиг после релиза
2. **Deny-списки для встроенных инструментов Claude Code.** Хук PreToolUse — рабочий, но хрупкий механизм. Альтернатива — отдавать `fs` через шлюз и запрещать встроенные `Read`/`Grep`. Решить в M3 по результатам эксперимента: если хук ненадёжен, переключаемся на шлюз
3. **Structured output через OpenRouter.** Поддержка `response_format` зависит от нижележащей модели, и данные `/models` не всегда точны. Решить в M2: доверять метаданным или всегда начинать с инструмента-обёртки для `openrouter` и подниматься до нативного режима только по явному `structured_mode: native`
3a. **Responses API у OpenAI.** Chat Completions стабилен, Responses API новее и лучше для tools. В v1 — Chat Completions; пересмотреть, если Go SDK сделает Responses основным
4. **Где хранить `state/`** при запуске из CI, где файловая система эфемерна. Вариант: `state` через `apis` (например, файл в репозитории или ключ в KV). В v1 — документировать ограничение
5. **Формат `commands.from`**: отдельный файл на стек или каталог с автоподключением по наличию `go.mod`/`package.json`. В v1 — явный файл
6. **Объём интерфейсов.** `forge/v1` из пяти операций может оказаться мал (нужны ли `get_pipeline_status`, `list_changes`?). Правило: интерфейс расширяется только когда операция понадобилась двум сценариям на двух форжах; до этого — прямые операции пака. Пересмотреть после первого месяца эксплуатации
7. **jq в паках как логика вне бинарника.** Компромисс принят осознанно: трансформы могут ошибаться и хуже тестируются, чем Go. Компенсация — обязательные `examples/` для `implements` и контрактные тесты в `baton-apis`. Если трансформы начнут расти в сторону условной логики — это сигнал вынести операцию в интерфейс с несколькими простыми паками, а не усложнять jq
