...
budget:
  ...
  time: 40m
  tokens: 200000        # суммарно input+output токенов за прогон

secrets: { ... }       # раздел 6
env:     { ... }       # общее окружение, см. ниже
apis:    { ... }       # раздел 7.4
commands:              # раздел 7.5
  test: { argv: ["go", "test", "./..."] }
...
- `env` — это окружение каждого процесса, который запускает прогон: шагов `run`, процессов агента и `commands`; собственная запись `env` того же имени у шага или команды переопределяет её. Формы записи такие же, как в `run.env` (литеральный шаблон или `{ secret: name }`), и действуют те же правила для секретов. MCP-серверы настраиваются в глобальном конфиге и её не получают.
...
Передача значений между шагами. Окружение шага собирается из сценарного `env` и его собственного `run.env`. Чтобы передать значение, вычисленное одним шагом, в другой, используйте шаблон в `env`:
...
При `parse: json` поля шага доступны так же, например `{{ .steps.cfg.result.x }}`. Шаги не экспортируют переменные неявно: изменения окружения в дочернем процессе никогда не передаются в следующие шаги.
...
- Запуск `codex exec --json` в неинтерактивном режиме. Промпт подаётся на stdin, чтобы длинный промпт не светился в таблице процессов; `system` дописывается перед ним отдельным блоком — флага для системного промпта у этого CLI нет
- Шлюз подключается как MCP-сервер в `config.toml`, который пишется в собственный `CODEX_HOME` прогона. Bearer-токен передаётся по имени переменной (`bearer_token_env_var = "BATON_GATEWAY_TOKEN"`), в файл он не попадает. `default_tools_approval_mode = "approve"` у этого сервера автоматически одобряет его инструменты: с выключенными подтверждениями вызов, запросивший одобрение, не ждёт, а падает
- Встроенные инструменты: shell нельзя отключить так, как `Bash` у `claude-code`, поэтому политику шага держит песочница — `sandbox_mode = "read-only"` либо `workspace-write` с `network_access = false` и единственным writable-корнем в workspace при `fs.write: workspace`. `web_search` выключен, `approval_policy = "never"`, чтобы шаг никогда не ждал человека, а `shell_environment_policy.inherit = "core"` не пускает секреты шага в команды, которые агент запускает изнутри
- Deny-списки путей внутри workspace этот движок не обеспечивает, `skills` отклоняются как неподдерживаемые
- Набор флагов меняется между релизами, поэтому адаптер читает `codex exec --help` и передаёт только то, что есть у установленной версии: `--ephemeral`, `--ignore-rules`, `--color never`, `--model`. `--ignore-user-config` сознательно не передаётся — он выбросил бы собственный `config.toml` прогона вместе со шлюзом. Версия проверяется при старте, неизвестная мажорная отклоняется
- Окружение процесса: пустое, плюс `PATH`, `HOME` и `CODEX_HOME` (один и тот же временный каталог, чтобы в прогон не попали пользовательская конфигурация и сохранённые учётные данные), и объявленный `env`. Ключ провайдера, соответственно, должен быть назван в `env` шага — если шаг не выставил `inherit_auth`
- `inherit_auth: true` копирует пользовательский `auth.json` из его `CODEX_HOME` (по умолчанию `~/.codex`) в собственный каталог прогона: именно так вместо API-ключа работает вход по подписке. Копия, а не ссылка — прогон, обновивший access-токен, не испортит логин; шаг, попросивший учётные данные, которых нет, падает ошибкой класса config. Любой движок, кроме `codex`, это поле отклоняет
- Завершение: как у `claude-code` — SIGTERM, через 10 с SIGKILL
- Ни `max_turns`, ни `budget_usd` не имеют в этом CLI ни флага, ни ключа конфига: шаг ограничен `max_tool_calls` шлюза и своим таймаутом. Стоимость CLI не сообщает, `cost_usd` остаётся null; токены суммируются по событиям `turn.completed`
...
Учёт стоимости: если провайдер вернул стоимость — она; иначе `usage × pricing[model]`; если модели нет в `pricing` — предупреждение при валидации и `cost_usd: null` в отчёте, бюджет по долларам для такого шага не проверяется, проверяется только общий лимит по токенам (`budget.tokens`).
...
- [x] `render` ограничен каталогом сценария: `render "../../etc/passwd"` отклоняется ошибкой рендера
