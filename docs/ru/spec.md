...
- Проверка примера пагинацию не эмулирует: она подаёт трансформу содержимое `examples/<op>.json` как есть. Поэтому для операции с `paginate: true` пример хранится уже склеенным — массивом элементов всех страниц, а не сырым телом одного ответа
